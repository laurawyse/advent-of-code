...
const totalMinForGuard = sortedObj => guardId => {
    const guardMinutes = sortedObj[guardId];
    return Object.keys(guardMinutes).reduce((total, minute) => total + guardMinutes[minute].length, 0);
}
...
const guardWithMost = Object.keys(sorted).reduce((highestGuardId, nextGuardId) => {
    const total = totalMinForGuard(sorted);
    return total(nextGuardId) > total(highestGuardId) ? nextGuardId : highestGuardId;
}, Object.keys(sorted)[0]);

const minuteWithMost = findMinuteWithMost(sorted, guardWithMost);

console.log(`Guard with most: ${guardWithMost}`);
// console.log(sorted[guardWithMost]);
console.log(`Minute with most: ${minuteWithMost}`)
console.log(`Answer: ${guardWithMost*minuteWithMost}`)
